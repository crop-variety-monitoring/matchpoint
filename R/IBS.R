...
workflow_rf  <- function(ref_file, fld_file, country, crop, year, outdir=".", MAF_cutoff=0.05, SNP_mr=0.2, sample_mr=0.2, IBS_cutoff=.8, biallelic=TRUE, missflags="-", inb_method = "mom.visscher", cpus=1) {
   ...
   rec <- recode_rf(crf, biallelic=biallelic, missflags=missflags)
   out <- match_rf(rec, MAF_cutoff=MAF_cutoff, SNP_mr=SNP_mr, sample_mr=sample_mr, IBS_cutoff=IBS_cutoff, inb_method =inb_method, cpus=cpus)
